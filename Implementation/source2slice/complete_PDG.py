...
http.socket_timeout = 99999999999999999
...
def get_pdg_relation():
    ...
    for node in all_func_node:
        ...
        # print "----------------------------init pdg---------------------------------------------"
        # for v in opt_pdg_1.vs():
        #     print v['name'],' : ',v['code'],v['type'],v['location']
        # for e in opt_pdg_1.es():
        #     print opt_pdg_1.vs[e.tuple[0]]['name'],' -----> ',opt_pdg_1.vs[e.tuple[1]]['name'],e['var']
        d_use, d_def = getUseDefVarByPDG(j, opt_pdg_1)#get use and def nodedict of every cfgnode
        # print "def-use\n",d_use,d_def
        opt_pdg_2 = modifyDataEdgeVal(opt_pdg_1)#not distinguish pointer and buffer it points,对于当前pdg中的所有边edge，将其var属性中的'*'属性去除

        opt_pdg_3 = completeDeclStmtOfPDG(opt_pdg_2, d_use, d_def, dict_if2cfgnode, dict_cfgnode2if)#add data edge from declare node to init node

        opt_pdg_4 = completeDataEdgeOfPDG(opt_pdg_3, d_use, d_def, dict_if2cfgnode, dict_cfgnode2if)#add data edge to get more info,from def to use
        # completeDataEdgeOfPDG:注重于已经初始化的Def点和其对应的Use点之间的依赖关系的增加
        # completeDeclStmtOfPDG:注重于未初始化Def和初始化点之间依赖关系的增加

        opted_pdg_5 = addDataEdgeOfObject(opt_pdg_4, dict_if2cfgnode, dict_cfgnode2if)# c++ node from def(new) to use
        # print "----------------------------pdg--------------------------------------------------"
        # for v in opted_pdg_5.vs():
        #     print v['name'],' : ',v['code'],v['type'],v['location']
        # for e in opted_pdg_5.es():
        #     print opted_pdg_5.vs[e.tuple[0]]['name'],' -----> ',opted_pdg_5.vs[e.tuple[1]]['name'],e['var']
