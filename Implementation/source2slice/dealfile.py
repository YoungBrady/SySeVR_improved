...
def dealfile_nvd(folder_path,diff_path):
    ...
    for type in os.listdir(folder_path):
        for software in os.listdir(folder_path,type):
            if software.startswith("."):
                continue
            for cve in os.listdir(os.path.join(folder_path,type,software)):
                if cve.startswith("."):
                    continue
                for filename in os.listdir(os.path.join(folder_path,type,software,cve)):
                    # print(filename)
                    filepath = os.path.join(folder_path,type,software,cve,filename)
                    # if 'linux' in folder:
                    ...
                    diff_filename=".".join(filename.split('.')[:-2])+".diff"
                    diffpath = os.path.join(diff_path,software,cve,diff_filename)
    ...
    with open('./vulline_dict.json','w+') as f:
        json.dump(vulline_dict,f)
...
if __name__ == "__main__":
    ...
    diff_path = '../../Program_data/diff2'
    #dealfunc_nvd(data_source1,diff_path)
    
    data_source2 = '../../Program_data/file2'
